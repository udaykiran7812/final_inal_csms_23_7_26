...
import { ArrowLeft, Clock, User, Building, Tag, UserCheck, CheckSquare } from 'lucide-react';
import { TicketStatus } from '../../types';
import { TicketInteractions } from '../../components/TicketInteractions';

export const StaffTicketDetails: React.FC = () => {
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
              ...
              <div className="flex items-center space-x-3 text-slate-600">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-slate-700">{ticket.assignedStaffName || 'Unassigned'}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm space-y-4">
            <h2 className="text-base font-bold text-slate-800 flex items-center space-x-2 border-b border-slate-100 pb-3">
              ...
              <span>Update Ticket Workflow Status</span>
            </h2>

            <div className="space-y-3">
              <p className="text-xs text-slate-500">
                Transition ticket progress state (ACCEPT, IN_PROGRESS, RESOLVED, or CLOSED).
              </p>
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-800 font-medium">
                📷 <strong>Compulsory Proof Photo Required:</strong> Staff members must upload a photo attachment of the completed or repaired work in the Attachments section below before marking a ticket as <strong>RESOLVED</strong> or <strong>CLOSED</strong>.
              </div>
              <div className="flex flex-wrap gap-2 pt-1">
                {(['ACCEPTED', 'IN_PROGRESS', 'RESOLVED', 'CLOSED'] as TicketStatus[]).map((status) => (
                  <button
                    key={status}
                  ...
                  >
                    {status === 'ACCEPTED' ? '1. Accept Ticket' :
                     status === 'IN_PROGRESS' ? '2. Start Progress' :
                     status === 'RESOLVED' ? '3. Resolve (Photo Required)' :
                     '4. Close (Photo Required)'}
                  </button>
                ))}
              </div>
            </div>
